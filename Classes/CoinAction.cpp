...
void CoinAction::step(float time) {
    if (_target) {
        ...
        if (_target->getPositionZ() > -40 && _target->getPositionZ() < 10)// enter the front
        {
            ...
            Vec3 vertices[8];
            auto player_aabb = player->getSprite3D()->getAABB();
            auto dist = sprite->getPosition3D().distance(player->getSprite3D()->getPosition3D());
...
CoinAction::CoinAction(SharkMan *player, cocos2d::Node *node) : angle(0), is_triggered(false) {
    this->player = player;
